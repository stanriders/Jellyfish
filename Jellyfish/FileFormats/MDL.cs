...
namespace Jellyfish.FileFormats
{
    ...
    public class MDL
    {
        ...
        public struct studiohdr_t
        {
            ...
            public float[] illumposition;   // ?? Presumably the point used for lighting when per-vertex lighting is not enabled.
    ...
    public class VTX
    {
        ...
        public List<BodyPartHeader_t> BodyParts { get; set; } = new List<BodyPartHeader_t>();
        public List<ModelHeader_t> Models { get; set; } = new List<ModelHeader_t>();
        public List<ModelLODHeader_t> Lods { get; set; } = new List<ModelLODHeader_t>();
        public List<MeshHeader_t> Meshes { get; set; } = new List<MeshHeader_t>();
        public List<StripGroupHeader_t> VertexStrips { get; set; } = new List<StripGroupHeader_t>();

        public List<MeshInfo> MeshInfos { get; set; } = new List<MeshInfo>();
        ...
        public static VTX Load(string path)
        {
            ...
            for (int i = 0; i < res.Header.numBodyParts; i++)
            {
                ...
                for (int j = 0; j < bodyPart.numModels; j++)
                {
                    ...
                    res.Lods.AddRange(DeserializeModelLODHeader_t(file, bpoffset + modelsoffset + model.lodOffset, model.numLODs));
                    for (int k = 0; k < model.numLODs; k++)
                    {
                        var lodsoffset = model.lodOffset * (k + 1);
                        var lod = res.Lods[i + j + k];

                        res.Meshes.AddRange(DeserializeMeshHeader_t(file, bpoffset + modelsoffset + lodsoffset + lod.meshOffset, lod.numMeshes));
                        for (int l = 0; l < lod.numMeshes; l++)
                        {
                            var meshessoffset = lod.meshOffset * (l + 1);
                            var mesh = res.Meshes[i + j + k + l];

                            res.VertexStrips.AddRange(DeserializeStripGroupHeader_t(file, bpoffset + modelsoffset + lodsoffset + meshessoffset + mesh.stripGroupHeaderOffset, 1));
                            for (int m = 0; m < 1; m++)
                            {
                                ...
                                var indOffset = bpoffset + modelsoffset + lodsoffset + meshessoffset + stripssoffset + strip.indexOffset;
                                for (int indIndex = 0; indIndex < strip.numIndices; indIndex++)
                                {
        ...
        private static List<StripGroupHeader_t> DeserializeStripGroupHeader_t(byte[] file, int offset = 0, int amt = 1)
        {
            ...
            for (int i = 0; i < amt; i++)
            {
                ret.Add(new StripGroupHeader_t
                {
                    ...
                    numStrips = BitConverter.ToInt32(file, offset + size * i + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int)),
                    stripOffset = BitConverter.ToInt32(file, offset + size * i + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int)),
                    flags = file[offset + size * i + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int)],
                });
                offset += size;
            }
        ...
        public struct StripHeader_t
        {
            ...
            /*unsigned*/ byte flags; // originally char, but c# char is 16 bit and file assumes 8 bit
        ...
        public struct Vertex_t
        {
            ...
            /*unsigned*/ byte[] boneWeightIndex; // originally char, but c# char is 16 bit and file assumes 8 bit
            /*unsigned*/ byte numBones; // originally char, but c# char is 16 bit and file assumes 8 bit
            ...
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            byte[] boneID;  // originally char, but c# char is 16 bit and file assumes 8 bit
        };
    }
